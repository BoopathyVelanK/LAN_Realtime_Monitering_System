...
/** Starts firing synthetic live events on an interval. Returns a stop
 * function. Mirrors the exact shape of what the real WebSocket topics
 * push, so the UI code that consumes this has zero branching for
 * mock-vs-real. */
export function startMockLiveFeed(handlers: MockLiveFeedHandlers): () => void {
  const interval = setInterval(() => {
    ...
    // Roughly every other tick, emit an alert; the others emit a status flip or risk update.
